...
BubbleTree.Bubbles.Donut = function(node, bubblechart, origin, radius, angle, color) {
	...
	this.draw = function() {
		...
		var qlabel2 = 0;
		if ((((me.node.parent.name == 'ovd') || (me.node.parent.name == 'vv'))  && ((me.node.name == 'sochelp')||(me.node.name == 'build')||(me.node.name == 'guide')||(me.node.name == 'hoz')||(me.node.name == 'fcp')||(me.node.name == 'sec')||(me.node.name == 'culture')||(me.node.name == 'migrant')||(me.node.name == 'ipoteka')||(me.node.name == 'oboronzakaz')||(me.node.name == 'olimp')||(me.node.name == 'passport')))||((me.node.parent.name == 'hospital')&&((me.node.name == 'olimp')||(me.node.name == 'sochelp')||(me.node.name == 'socpol')))||((me.node.parent.name == 'epidem')&&((me.node.name == 'sochelp')||(me.node.name == 'socpol')))||((me.node.parent.name == 'hiedu')&&((me.node.name == 'sochelp')||(me.node.name == 'socpol')||(me.node.name == 'build')))) qlabel2 = 1;

		//me.label.attr({ x: me.pos.x, y: me.pos.y, 'font-size': Math.max(4, me.bubbleRad * me.bc.bubbleScale * 0.25) });
		if (!showLabel) {			
			me.label.hide();
			if (qlabel2 == 1) {me.label2.hide()} else {me.label2.show()}
		} else {
			me.label.show();
			if (r < 40) {
				me.label.find('.desc').hide();
	...
	me.show = function() {
		...
		// additional label
		me.label2 = $('<div class="label2 '+me.node.id+'"><span>'+me.node.shortLabel+'</span><div class="amount">'+utils.formatNumber(me.node.amount)+'</div></div>');
...
BubbleTree.Bubbles.Icon = function(node, bubblechart, origin, radius, angle, color) {
	...
	me.show = function() {
		...
		me.label = $('<div class="label '+me.node.id+'"><div class="desc">'+me.node.shortLabel+'</div><div class="amount">'+utils.formatNumber(me.node.amount)+'</div></div>');
		me.bc.$container.append(me.label);
	...
	me.draw = function() {
		...
		qlabel2 = 0;
		if ((me.node.name == 'prof')||(me.node.name == 'retraining')) qlabel2 = 1;

		//me.label.attr({ x: me.pos.x, y: me.pos.y, 'font-size': Math.max(4, me.bubbleRad * me.bc.bubbleScale * 0.25) });
		if (!showLabel) {
			me.label.hide();
			if (qlabel2 == 0){me.label2.show();}else me.label2.hide();
		} else {
			me.label.show();
			if ((showIcon && r < 70) || (!showIcon && r < 40)) {
				me.label.find('.desc').hide();
				me.label2.find('.amount').hide();
				me.label2.show();
			} else {
				// full label
				me.label.find('.desc').show();
